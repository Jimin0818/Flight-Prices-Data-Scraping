...
class Flight:
  ...
  def __repr__(self):
    return "Flight(id:{id}, {org}-->{dest} on {date})".format(id=self._id,
                                                              org=self._origin,
                                                              dest=self._dest,
                                                              date=self._date)
  ...
  def _classify_arg(self, arg):
    ...
    elif len(arg) == 6 and arg.isupper(
    ) and self._origin is None and self._dest is None:
      # origin/dest
      self._origin = arg[:3]
      self._dest = arg[3:]
    elif ('hr' in arg and arg[-3:].isupper()) or (len(arg.split(', ')) > 1
                                                  and arg.isupper()):
      # 1 stop + time at stop
      # or multiple stops
      self._stops = arg
        ...
        arg
    ) > 0 and arg != 'Separate tickets booked together' and arg != 'Change of airport':
      val = arg.split(',')
  ...
  def assert_error(x, arg):
    return [
        ...
        "Parsing Arg 1 as Date Return elem is incorrect.", -1, -1, -1,
