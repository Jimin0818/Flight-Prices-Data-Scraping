...
def ScrapeObjects(objs, deep_copy=False):
  ...
  )  # check if chromedriver is installed correctly and on path
  driver = webdriver.Chrome()
  driver.maximize_window()

  # modifies the objects in-place
  debug = [
      obj._scrape_data(driver) for obj in tqdm(objs, desc="Scraping Objects")
  ]
...
class _Scrape:
  ...
  def __add__(self, other):
    ...
    assert self.type == other.type, "Can't add {a} with {b}. See docs".format(
        a=self.type, b=other.type)

    assert (self.data.empty and other.data.empty) or (
        not self.data.empty and not other.data.empty
    ), "Error with addition. Both queries must either be unused or queried."
    ...
    if obj_type == 'one-way':
      ...
      if self.data.empty:
        return Scrape(
            *self.unpack([[self.origin[i], self.dest[i], date]
                          for i, date in enumerate(self.date)]),
            *self.unpack([[other.origin[i], other.dest[i], date]
                          for i, date in enumerate(other.date)]))
      else:
        obj = Scrape(
            *self.unpack([[self.origin[i], self.dest[i], date]
                          for i, date in enumerate(self.date)]),
      ...
      assert datetime.strptime(self.date[1], date_format) < datetime.strptime(
          other.date[0], date_format
      ), "Dates are not in order. Make sure to provide them in increasing order in YYYY-MM-DD format."

      # check perfect chain
      if self.origin[0] == other.origin[0]:
        if self.data.empty:
          return Scrape(
              *self.unpack([[self.origin[i], date]
                            for i, date in enumerate(self.date)]),
              *self.unpack([[other.origin[i], date]
                            for i, date in enumerate(other.date)] +
                           [[other.dest[-1]]]))
        else:
          obj = Scrape(
  ...
  def _set_properties(self, *args):
    ...
    # one way
    if len(args) == 3:
      assert len(args[0]) == 3 and type(
          args[0]) == str, "Issue with arg 0, see docs"
      assert len(args[1]) == 3 and type(
          args[1]) == str, "Issue with arg 1, see docs"
      assert len(args[2]) == 10 and type(
          args[2]) == str, "Issue with arg 2, see docs"
    ...
    # round-trip
      ...
      assert len(args[2]) == 10 and type(
          args[2]) == str, "Issue with arg 2, see docs"
      assert len(args[3]) == 10 and type(
          args[3]) == str, "Issue with arg 3, see docs"

      assert datetime.strptime(args[2], date_format) < datetime.strptime(
          args[3], date_format
      ), "Dates are not in order. Make sure to provide them in increasing order in YYYY-MM-DD format."

      self._origin, self._dest, self._date = [args[0],
                                              args[1]], [args[1],
                                                         args[0]], args[2:]

      assert len(self._origin) == len(self._dest) == len(
          self._date), "Issue with array lengths, talk to dev"
      self._url = self._make_url()
      self._type = 'round-trip'

    # chain-trip, chain is component of 3s, check that last one is an actual date to not confuse w perfect
    elif len(args) >= 3 and len(args) % 3 == 0 and len(
        args[-1]) == 10 and type(args[-1]) == str:
      self._origin, self._dest, self._date = [], [], []

      for i in range(0, len(args), 3):
        assert len(args[i]) == 3 and type(
            args[i]) == str, "Issue with arg {}, see docs".format(i)
        assert len(args[i + 1]) == 3 and type(
            args[i + 1]) == str, "Issue with arg {}, see docs".format(i + 1)
        assert len(args[i + 2]) == 10 and type(
            args[i + 2]) == str, "Issue with arg {}, see docs".format(i + 2)

        if i > 0:
          assert datetime.strptime(
              self._date[-1], date_format
          ) < datetime.strptime(
              args[i + 2], date_format
          ), "Dates are not in order ({d1} > {d2}). Make sure to provide them in increasing order in YYYY-MM-DD format.".format(
    ...
    # perfect-chain
    elif len(args) >= 4 and len(args) % 2 == 1 and len(args[-1]) == 3 and type(
        args[-1]) == str:
      assert len(args[0]) == 3 and type(
          args[0]) == str, "Issue with arg 0, see docs"
      assert len(args[1]) == 10 and type(
          args[1]) == str, "Issue with arg 1, see docs"
      ...
      for i in range(2, len(args) - 1, 2):
        assert len(args[i]) == 3 and type(
            args[i]) == str, "Issue with arg {}, see docs".format(i)
        assert len(args[i + 1]) == 10 and type(
            args[i + 1]) == str, "Issue with arg {}, see docs".format(i + 1)
        assert datetime.strptime(
            self._date[-1], date_format
        ) < datetime.strptime(
            args[i + 1], date_format
        ), "Dates are not in order ({d1} > {d2}). Make sure to provide them in increasing order in YYYY-MM-DD format.".format(
      ...
      assert len(args[-1]) == 3 and type(
          args[-1]) == str, "Issue with last arg, see docs"
      self._dest += [args[-1]]
  ...
  def origin(self, x: str) -> None:
    assert self._data.shape[
        0] == 0, "Can't set origin after query has been completed."
    self._origin = x
  ...
  def dest(self, x: str) -> None:
    assert self._data.shape[
        0] == 0, "Can't set destination after query has been completed."
    self._dest = x
  ...
  def date(self, x: str) -> None:
    assert self._data.shape[
        0] == 0, "Can't set date after query has been completed."
    self.date = x
  ...
  def _scrape_data(self, driver):
    results = [
        self._get_results(url, self._date[i], driver)
        for i, url in enumerate(self._url)
    ]
  ...
  def _make_url(self):
    ...
    for i in range(len(self._date)):
      urls += [
          'https://www.google.com/travel/flights?hl=en&q=Flights%20to%20{org}%20from%20{dest}%20on%20{date}%20oneway'
          .format(dest=self._dest[i], org=self._origin[i], date=self._date[i])
      ]
    return urls
  ...
  def _clean_results(result, date):
    ...
    matches = [
        i for i, x in enumerate(res3) if len(x) > 2 and
        ((x[-2] != '+' and
          (x.endswith('PM') or x.endswith('AM')) and ':' in x) or x[-2] == '+')
    ][::2]
    flights = [
        Flight(date, res3[matches[i]:matches[i + 1]])
